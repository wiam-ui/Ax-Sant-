        ...
        <div class="main">
                    ...
                    <tbody>
                        @foreach($data as $data)
                         <tr>
                            <td>{{$data->id}}</td>
                            <td>{{$data->nom}}</td>
                            <td>{{$data->prexcription}}</td>
                            <td>{{$data->quantite}}</td>

                            <td><a href="{{url('edit_medicament',$data->id)}}" ><i class="fa-solid fa-file-pen"></i></a></td>
                            <td><a href="{{url('delete_medicament',$data->id)}}" onclick="confirmation(event)"><ion-icon name="trash" id="ion-icon"></ion-icon></a></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div id="overlay" class="overlay">
                <div class="modal">
                    ...
                    <h3>Formulaire de medicament</h3>
                    <form action="{{url('add_medicament')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label for="nom">Nom :</label>
                        <input type="text" id="nom" name="nom" placeholder="saisir le nom" required><br>

                        <label for="">prexcription:</label>
                        <input type="text" name="prexcription" placeholder="saisir la prexcription" required><br>

                        <label for="">Quantite :</label>
                        <input type="number" name="quantite" placeholder="saisir la quantite" required><br>
